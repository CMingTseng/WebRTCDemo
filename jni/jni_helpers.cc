...
jlong jlongFromPointer(void* ptr) {
    ...
    CHECK(reinterpret_cast<void*>(ret) == ptr, "jlong does not convert back to pointer");
    return ret;
}
...
ClassReferenceHolder::ClassReferenceHolder(JNIEnv* jni, const char** classes, int size) {
...
void ClassReferenceHolder::FreeReferences(JNIEnv* jni) {
    for (std::map<std::string, jclass>::const_iterator it = classes_.begin(); it != classes_.end(); ++it) {
