...
public class VoiceEngine {
    ...
    public enum NsModes {
        UNCHANGED, DEFAULT, CONFERENCE, LOW_SUPPRESSION, MODERATE_SUPPRESSION, HIGH_SUPPRESSION, VERY_HIGH_SUPPRESSION
    }

    // Keep in sync (including this comment) with
    // webrtc/common_types.h:AgcModes
    public enum AgcModes {
        UNCHANGED, DEFAULT, ADAPTIVE_ANALOG, ADAPTIVE_DIGITAL, FIXED_DIGITAL
    }
    ...
    public enum EcModes {
        UNCHANGED, DEFAULT, CONFERENCE, AEC, AECM
    }
    ...
    public static class AgcConfig {
        AgcConfig(int targetLevelDbOv, int digitalCompressionGaindB, boolean limiterEnable) {
    ...
    public int startRtpDump(int channel, String file, RtpDirections direction) {
        return startRtpDump(channel, file, direction.ordinal());
    }

    private native int startRtpDump(int channel, String file, int direction);
